...
namespace GMCPIL
{
	...
	public class Config : Object, Json.Serializable
	{
		...
		public static Config from_file(string filename) throws Error
		{
			...
			config = Json.gobject_deserialize(typeof(Config), root) as Config;
			...
			stream.close();
			Profile.profiles[ProfileType.CUSTOM] = config.features;
			return config;
		}
		...
		public void set_env()
		{
			...
			for (int i = 0; i < Profile.features.length; i++)
			{
				...
				enabled = Profile.profiles[profile].get_data<bool>(name);
				if (enabled)
				{
